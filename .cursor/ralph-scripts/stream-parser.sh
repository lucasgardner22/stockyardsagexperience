...
process_line() {
  ...
  # Parse JSON type (use here-strings to avoid broken pipe errors)
  local type=$(jq -r '.type // empty' 2>/dev/null <<< "$line") || return
  local subtype=$(jq -r '.subtype // empty' 2>/dev/null <<< "$line") || true
  
  case "$type" in
    "system")
      if [[ "$subtype" == "init" ]]; then
        local model=$(jq -r '.model // "unknown"' 2>/dev/null <<< "$line") || model="unknown"
    ...
    "assistant")
      ...
      local text=$(jq -r '.message.content[0].text // empty' 2>/dev/null <<< "$line") || text=""
      if [[ -n "$text" ]]; then
        local chars=${#text}
    ...
    "tool_call")
      if [[ "$subtype" == "started" ]]; then
        ...
        if jq -e '.tool_call.readToolCall.result.success' > /dev/null 2>&1 <<< "$line"; then
          local path=$(jq -r '.tool_call.readToolCall.args.path // "unknown"' 2>/dev/null <<< "$line") || path="unknown"
          local lines=$(jq -r '.tool_call.readToolCall.result.success.totalLines // 0' 2>/dev/null <<< "$line") || lines=0
          
          local content_size=$(jq -r '.tool_call.readToolCall.result.success.contentSize // 0' 2>/dev/null <<< "$line") || content_size=0
          local bytes
        ...
        elif jq -e '.tool_call.writeToolCall.result.success' > /dev/null 2>&1 <<< "$line"; then
          local path=$(jq -r '.tool_call.writeToolCall.args.path // "unknown"' 2>/dev/null <<< "$line") || path="unknown"
          local lines=$(jq -r '.tool_call.writeToolCall.result.success.linesCreated // 0' 2>/dev/null <<< "$line") || lines=0
          local bytes=$(jq -r '.tool_call.writeToolCall.result.success.fileSize // 0' 2>/dev/null <<< "$line") || bytes=0
        ...
        elif jq -e '.tool_call.shellToolCall.result' > /dev/null 2>&1 <<< "$line"; then
          local cmd=$(jq -r '.tool_call.shellToolCall.args.command // "unknown"' 2>/dev/null <<< "$line") || cmd="unknown"
          local exit_code=$(jq -r '.tool_call.shellToolCall.result.exitCode // 0' 2>/dev/null <<< "$line") || exit_code=0
          
          local stdout=$(jq -r '.tool_call.shellToolCall.result.stdout // ""' 2>/dev/null <<< "$line") || stdout=""
          local stderr=$(jq -r '.tool_call.shellToolCall.result.stderr // ""' 2>/dev/null <<< "$line") || stderr=""
    ...
    "result")
      local duration=$(jq -r '.duration_ms // 0' 2>/dev/null <<< "$line") || duration=0
      local tokens=$(calc_tokens)
